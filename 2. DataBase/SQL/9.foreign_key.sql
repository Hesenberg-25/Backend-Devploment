...
-- Here we have created a New Table called as 'address' and we want that CONNECT the two Tables 
-- The connection is established by the use of FOREIGN KEY 
-- CONSTRAINT {constraint-name} FOREIGN KEY (variable-name-we-want-to-connect) REFERENCES {parent-table-name(variable-name-to-which-we-want-to-connect)}

-- So basically FOREIGN KEY is used to established the connection between two Tables when we donot want to Extend the TABLE with Column
