...
trueLogger=logging.getLogger(__name__)
trueLogger.setLevel(logging.DEBUG)
trueHandler=logging.FileHandler('TrueBalance.log')
trueFormatter=logging.Formatter('%(asctime)s:%(name)s:%(levelname)s:%(message)s')
trueHandler.setFormatter(trueFormatter)
trueLogger.addHandler(trueHandler)
...
def get_monthly_expense_logic(month_name: str) -> float:
    ...
    try:
        with get_db_connection() as conn:
            ...
            trueLogger.info('Successfully fetched Monthly Expenses')
...
async def add_expense_endpoint(category_name: str = Form(...), amount: float = Form(...)):
    ...
    try:
        with get_db_connection() as conn:
            ...
            conn.commit()
            trueLogger.info(f'Expense on {str(date_today)} saved for {category_name} of {amount}')
            return {
                "status": "success", 
    ...
    try:
        with get_db_connection() as conn:
            ...
            if existing:
                ...
                trueLogger.info(f"Budget of {budget} for {month_name} updated successfully!")
...
async def check_budget_endpoint(month_name: str):
    ...
    try:
        ...
        if actual_expense < setted_budget:
            trueLogger.info(f"For month of {month_name} user under budget")
            status = "under"
            msg = f"Hushhh!! You still have {remaining} to spend. Use them wisely."
        elif actual_expense > setted_budget:
            trueLogger.info(f"For month of {month_name} user over budget")
            status = "over"
            msg = f"Warning!! You have exceeded your budget by {abs(remaining)}."
        else:
            trueLogger.info(f"For month of {month_name} user has used all set budget")
            status = "exact"
